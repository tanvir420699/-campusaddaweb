...
async function insertStory(payload, mentions=[], includeUsers=[], excludeUsers=[]){
  ...
  const id=data.id;
  try{
    if(mentions.length) await sb.from('story_mentions').insert(mentions.map(m=>({story_id:id, user_id:m.uuid, x:m.x||0.5, y:m.y||0.5})));
...
function makeDraggable(node,parent,onMove){
  ...
  const start=(e)=>{
    if(node.getAttribute('contenteditable')==='true' && document.activeElement===node) return;
    dragging=true;
  ...
  const move=(e)=>{
    ...
    const p=e.touches?e.touches[0]:e;
    const r=parent.getBoundingClientRect();
    const nx=Math.max(0,Math.min(r.width, ox+(p.clientX-sx)));
    const ny=Math.max(0,Math.min(r.height,oy+(p.clientY-sy)));
